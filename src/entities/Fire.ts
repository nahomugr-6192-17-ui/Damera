...
export class Fire {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    this.promptText = scene.add.text(0, -FIRE_RADIUS - 20, "Add Wood to \u12f3\u1218\u122b", {
      fontSize:"14px", fontFamily:'"Noto Sans Ethiopic",Nunito,sans-serif',
      color:"#ffffff", backgroundColor:"#cc4400cc", padding:{x:8,y:4},
    }).setOrigin(0.5,1).setVisible(false).setDepth(25).setScrollFactor(0);

    this.container = scene.add.container(x, y, [this.glow,...this.poles,this.outerFlame,this.midFlame,this.coreFlame]);
    this.container.setDepth(8);
    this.container.setScale(FIRE_MIN_SCALE);
    this.startFlicker();
  }

  /** Grow fire based on number of woods delivered — called from GameScene.deliverWood() */
  ...
  update(playerX: number, playerY: number, carryingWood: boolean): boolean {
    ...
    if (inRange) {
      ...
      this.promptText.setPosition(this.x - cam.scrollX, this.y - cam.scrollY - FIRE_RADIUS * this.baseScale - 16);
    }
    return inRange;
  }
  ...
  extinguish(): void {
    ...
    this.scene.tweens.add({ targets:this.container, scale:FIRE_MIN_SCALE*0.4, alpha:0.4, duration:900, ease:"Power2" });
  }

  destroy(): void { this.container.destroy(); this.promptText.destroy(); }

  private startFlicker(): void {
    this.scene.tweens.add({ targets:this.outerFlame, scaleX:{from:1,to:0.88}, scaleY:{from:1,to:0.80}, alpha:{from:1,to:0.75}, duration:280, yoyo:true, repeat:-1, ease:"Sine.easeInOut" });
    this.scene.tweens.add({ targets:this.coreFlame,  scaleX:{from:1,to:1.2},  scaleY:{from:1,to:0.85}, duration:200, yoyo:true, repeat:-1, ease:"Sine.easeInOut", delay:80 });
    this.scene.tweens.add({ targets:this.glow, alpha:{from:0.18,to:0.35}, duration:600, yoyo:true, repeat:-1, ease:"Sine.easeInOut" });
  }
}